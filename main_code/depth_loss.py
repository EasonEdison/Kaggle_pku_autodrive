...
model_path = '/test_depth/model_fld_3_islastepoch_1_date_17_01_time_10_58.pth'
Train = True
...
path_out_dict = {'Kaggle': './',
                 ...
                 'local': f"/home/ql-b423/Desktop/TXH/RB/Output_eff_{date_time_str}"}
platform='local'
...
def criter_metric(prediction, mask, regr, size_average=True):
    ...
    # Regression L1 loss
    if depth_loss_control:
        trans_depth = prediction[:,1:8]
        # this is really z !!!
        trans_depth[:,-1] = 1 / torch.sigmoid(prediction[:,-1]) - 1
        regr_loss = (torch.abs(trans_depth - regr).sum(1) * mask).sum(1).sum(1) / mask.sum(1).sum(1)
    else:
        pred_regr = prediction[:, 1:8]
