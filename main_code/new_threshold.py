...
model_path = '/home/ql-b423/Desktop/TXH/NND/test_bin_2_3_pi_new_weight_173_50_none/test_bin_2_3_pi_new_weight_173_50_none_final.pth'
Train = False
...
w_roll = 0.3
##########################
output_path = '/home/ql-b423/Desktop/TXH/RB/' + out_name
...
def build_model(drop_rate):
    ...
    print("device:", device)
    model = MyUNet(num_class, drop_rate).to(device)
...
def criter_metric(prediction, mask, regr, rot_bin, rot_res, size_average=True):
    ...
    if depth_loss_control:
        prediction[:,6] = 1 / torch.sigmoid(prediction[:,6]) - 1
...
def evaluate(model, epoch, history=None):
    ...
    with torch.no_grad():
        for img_batch, mask_batch, regr_batch,rot_bin_batch,rot_res_batch,  _ in dev_loader:
            ...
            val_loss_batch, mask_loss_batch, regr_loss_batch,roll_loss_batch = criter_metric(output, mask_batch, regr_batch,rot_bin_batch,rot_res_batch,
                                                                             size_average=False)
...
def train_eval_model(model, epoch, best_val_loss, history=None):
    ...
    #[img, mask, regr, rot_bin, rot_res, dropmask]
    for batch_idx, (img_batch, mask_batch, regr_batch, rot_bin_batch, rot_res_batch, _) in enumerate(tqdm(train_loader)):
        img_batch = img_batch.to(device)
        mask_batch = mask_batch.to(device)
        regr_batch = regr_batch.to(device)
        rot_bin_batch = rot_bin_batch.to(device)
        ...
        with torch.no_grad():
            ...
            for out, test_mask in zip(output, test_mask_batch):
                ...
                if isinstance(test_mask, np.ndarray):
                    ...
                    test_mask = cv2.resize(test_mask, (128, 40))[..., 0]
...
if test_control:
    ...
    numcars = pd.DataFrame(
        {'ImageId': sub['ImageId'].values, 'numcars': [int((str(x).count(' ') + 1) / 7) for x in sub['PredictionString']]})
    numcars_before = pd.DataFrame({'ImageId': sub_before['ImageId'].values,
                                   'numcars': [int((str(x).count(' ') + 1) / 7) for x in sub_before['PredictionString']]})
