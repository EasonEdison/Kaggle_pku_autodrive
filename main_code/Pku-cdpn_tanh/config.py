...
class cfg():
    def __init__(self):
        ...
        self.parser.add_argument('--exp_mode', type=str, help='', default='test')
        self.parser.add_argument('--cfg', type=str, help='path/to/configure_file', default='cfg.yaml')
        self.parser.add_argument('--dataset', type=str, help='', default='lmo')
        self.parser.add_argument('--object',  type=str, help='', default='driller')
    ...
    def parse(self):
        ...
        config = update_config_from_file(config, config_file,
                                         check_necessity=False)  # update arguments from config file
        ...
        # save path
        config.pytorch['save_path'] = os.path.join(ref.exp_dir,
                                                   '{}_{}'.format(config.pytorch.dataset, config.pytorch.object))
        config.pytorch['save_csv_path'] = os.path.join(config.pytorch['save_path'],
                                                       '{}_{}.csv'.format(config.pytorch.dataset,
                                                                          config.pytorch.object))
