...
out_name = 'test_branch_depth_10'
model_path = '/OP_test_branch_depth_10_0.044/model_fld_3_islastepoch_1_epoch_10.pth'
Train = False
...
def _regr_preprocess(regr_dict):
    ...
    if depth_loss_control:
        regr_dict['z'] = 1 / (sigmoid_(regr_dict['z']) + 1e-12) -1
...
def criter_metric(prediction, mask, regr, size_average=True):
    ...
    # Regression L1 loss
    if depth_loss_control:
        trans_depth = prediction[:,1:]
        # this is really z !!!
        trans_depth[:,2] = 1 / (torch.sigmoid(prediction[:,3]) + 1e-12) - 1
        regr_loss = (torch.abs(trans_depth - regr).sum(1) * mask).sum(1).sum(1) / mask.sum(1).sum(1)
    else:
