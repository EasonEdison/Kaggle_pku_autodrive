...
batch_size = 1
num_epochs = 10
num_epochs_resume = 20
start_epoch = 10
device = torch.device("cuda:1")
out_name = 'test_branch_1600_epoch_add'
model_path = '/home/ql-b423/Desktop/TXH/RB/OP_test_branch_1600_s63/model_fld_3_islastepoch_1_epoch_10.pth'
Train = True
...
def extract_coords(prediction, threshold=0):
    ...
    coords = []
    for r, c in points:
        regr_dict = dict(zip(regr_names, regr_output[:, r, c]))
...
class MyUNet(nn.Module):
    ...
    def __init__(self, t_num, r_num, drop_rate):
...
def build_model(drop_rate):
    ...
    print("device:", device)
    model = MyUNet(T_class,R_class, drop_rate).to(device)
...
def evaluate(model, epoch, history=None):
    model.eval()
    val_loss, mask_loss, regr_loss = torch.tensor(0., requires_grad=False), torch.tensor(0.,
    ...
    with torch.no_grad():
        for img_batch, mask_batch, regr_batch, _ in dev_loader:
            ...
            val_loss_batch, mask_loss_batch, regr_loss_batch = criter_metric(output, mask_batch, regr_batch,
                                                                             size_average=False)
