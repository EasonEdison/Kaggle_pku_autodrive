...
################################
img_h, img_w = 480, 1024
batch_size = 2
num_epochs = 10
num_epochs_resume = 0
start_epoch = 0
device = torch.device("cuda:0")
out_name = 'test_branch_pitch'
model_path = '/OP_test_branch_pitch_s32/model_fld_3_islastepoch_1_epoch_10.pth'
Train = False

T_class = 3 + 1  # mask也先放到这里,xyz
R_class = 2  # yaw roll
Pitch_class = 3 + 2 * 3  # class(3) bin(2*3)

use_class_weigth = False
w_mask = 0.1
w_regr = 0.7
w_pitch = 0.3
##########################
...
path_dict = {'Kaggle': '../input/pku-autonomous-driving/',
             ...
             'local': '/home/ql-b423/Desktop/TXH/pku-autonomous-driving'}
...
path_out_dict = {'Kaggle': './',
                 ...
                 'local': f"/home/ql-b423/Desktop/TXH/RB/OP_{out_name}"}
platform = 'local'
...
def get_alpha_pitch(output):

    pitch_one_class = np.zeros([output.shape[1],output.shape[2]])

    pitch_class = output[0:3]
    index = np.argmax(pitch_class, axis=0)

    index_0 = np.argwhere(index==0)
    index_1 = np.argwhere(index==1)
    index_2 = np.argwhere(index==2)

    pitch_regr = np.stack((output[3:5], output[5:7], output[7:9]),axis=0) # 3 2 60 128

    pitch = np.arctan(pitch_regr[:,0,:,:] / (pitch_regr[:,1,:,:]+ 1e-12))

    if index_0.shape[0] > 0:
        for i in range(index_0.shape[0]):
            pitch_one_class[index_0[i][0],index_0[i][1]] = pitch[0,index_0[i][0],index_0[i][1]] - (2 /3 *np.pi)

    if index_1.shape[0] > 0:
        for i in range(index_1.shape[0]):
            pitch_one_class[index_1[i][0],index_1[i][1]] = pitch[1,index_1[i][0],index_1[i][1]]

    if index_2.shape[0] > 0:
        for i in range(index_2.shape[0]):
            pitch_one_class[index_2[i][0],index_2[i][1]] = pitch[2,index_2[i][0],index_2[i][1]] + (2 /3 *np.pi)





    return pitch_one_class


def extract_coords(prediction, threshold=0):
    logits = prediction[0]
    regr_output = prediction[1:6]
    pitch_output = prediction[6:]
    points = np.argwhere(logits > threshold)  # >0
    # regr_names = ['x', 'y', 'z', 'yaw', 'pitch_cos', 'pitch_sin', 'roll']
    regr_names = ['x', 'y', 'z', 'yaw', 'roll', 'pitch']
    pitch = get_alpha_pitch(pitch_output)[None, :, :]
    col_names = sorted(['x', 'y', 'z', 'yaw', 'pitch_sin', 'pitch_cos', 'roll'])
    coords = []
    for r, c in points:
        regr_dict = dict(zip(regr_names, np.concatenate((regr_output[:, r, c], pitch[:, r, c]), axis=0)))
...
def get_pitch_class_regr(pitch):
    if pitch < -1.5:
        pitch_class = 0
    elif pitch > 1.5:
        pitch_class = 2
    ...
    pitch_regr = [pitch + 2 / 3 * np.pi, pitch, pitch - 2 / 3 * np.pi]

    return pitch_class, pitch_regr
...
def get_mask_and_regr(img_orig_height, img_orig_width, labels):
    ...
    regr_names = ['x', 'y', 'z', 'yaw', 'roll']

    regr = np.zeros([img_h // model_scale, img_w // model_scale, 5], dtype='float32')
    pitch_class = np.zeros([img_h // model_scale, img_w // model_scale, 1], dtype='long')
    pitch_regr = np.zeros([img_h // model_scale, img_w // model_scale, 3], dtype='float32')
    ...
    for x, y, regr_dict in zip(xs, ys, coords):
        ...
        if x >= 0 and x < img_h // model_scale and y >= 0 and y < img_w // model_scale:
            ...
            aaa, bbb = get_pitch_class_regr(regr_dict['pitch'])
            pitch_class[x, y], pitch_regr[x, y] = get_pitch_class_regr(regr_dict['pitch'])
            regr[x, y] = [regr_dict[n] for n in regr_names]
    return mask, regr, pitch_class, pitch_regr
...
class CarDataset(Dataset):
    ...
    def __init__(self, dataframe, root_dir, root_dir_dropmasks=osj(path, 'test_masks/{}.jpg'), training=True,
                 transform=None):
        self.df = dataframe
    ...
    def __getitem__(self, idx):
        ...
        if self.training:
            mask, regr, pitch_class, pitch_regr = get_mask_and_regr(img_orig_h, img_orig_w, labels)
            regr = np.rollaxis(regr, 2, 0)
            pitch_class = np.rollaxis(pitch_class, 2, 0)
            pitch_regr = np.rollaxis(pitch_regr, 2, 0)
            dropmask = 0
        else:
            mask, regr, pitch_class, pitch_regr = 0, 0, 0, 0
        ...
        pitch_class = torch.as_tensor(pitch_class, dtype=torch.long)
        pitch_regr = torch.as_tensor(pitch_regr, dtype=torch.float32)
        dropmask = torch.as_tensor(dropmask, dtype=torch.float32)

        return [img, mask, regr, pitch_class, pitch_regr, dropmask]
...
class MyUNet(nn.Module):
    ...
    def __init__(self, t_num, r_num, pitch_num, drop_rate):
        ...
        if effnet_ver == 'b0':
            self.up1_t = up(1282 + 1024, 512)
        elif effnet_ver == 'b2':
            self.up1_t = up(1410 + 1024, 512)
        elif effnet_ver == 'b4':
            self.up1_t = up(1794 + 1024, 512)
        elif effnet_ver == 'b5':
            self.up1_t = up(2050 + 1024, 512)

        if effnet_ver == 'b0':
            self.up1_r = up(1282 + 1024, 512)
        elif effnet_ver == 'b2':
            self.up1_r = up(1410 + 1024, 512)
        elif effnet_ver == 'b4':
            self.up1_r = up(1794 + 1024, 512)
        elif effnet_ver == 'b5':
            self.up1_r = up(2050 + 1024, 512)

        if effnet_ver == 'b0':
            self.up1_pitch = up(1282 + 1024, 512)
        elif effnet_ver == 'b2':
            self.up1_pitch = up(1410 + 1024, 512)
        elif effnet_ver == 'b4':
            self.up1_pitch = up(1794 + 1024, 512)
        elif effnet_ver == 'b5':
            self.up1_pitch = up(2050 + 1024, 512)

        self.up2_t = up(512 + 512, 256)
        self.up2_r = up(512 + 512, 256)
        self.up2_pitch = up(512 + 512, 256)

        self.outT = nn.Conv2d(256, self.t_num, 1)
        self.outR = nn.Conv2d(256, self.r_num, 1)
        self.outPitch = nn.Conv2d(256, self.pitch_num, 1)
    ...
    def forward(self, x):
        ...
        x_t = self.up1_t(feats, x4)
        x_t = self.up2_t(x_t, x3)
        x_t = self.outT(x_t)

        x_r = self.up1_r(feats, x4)
        x_r = self.up2_r(x_r, x3)
        x_r = self.outR(x_r)

        x_pitch = self.up1_pitch(feats, x4)
        x_pitch = self.up2_pitch(x_pitch, x3)
        x_pitch = self.outPitch(x_pitch)

        x = torch.cat((x_t, x_r, x_pitch), dim=1)
        return x
...
def build_model(drop_rate):
    ...
    print("device:", device)
    model = MyUNet(T_class, R_class, Pitch_class, drop_rate).to(device)
    # set_dropout(model, drop_rate=drop_rate)
    return model


def criter_metric(prediction, mask, regr, pitch_class, pitch_regr, size_average=True):
    # Binary mask loss
    ...
    # Regression L1 loss
    pred_regr = prediction[:, 1:6]
    regr_loss = (torch.abs(pred_regr - regr).sum(1) * mask).sum(1).sum(1) / mask.sum(1).sum(1)
    regr_loss = regr_loss.mean(0)

    pitch_loss = compute_pitch_loss(prediction[:, 6:], pitch_class, pitch_regr, mask)

    # Sum
    loss = mask_loss + regr_loss + pitch_loss
    if not size_average:
        loss *= prediction.shape[0]
    return loss, mask_loss, regr_loss, pitch_loss
...
def compute_class_loss(output, target, mask):
    mask = mask[:, None, :, :].expand_as(output)
    output = output * mask.float()
    # 计算真实值和预测值的交叉熵，为什么要两个值，p 1-p 不就行了？
    # return F.cross_entropy(output, target, reduction='mean')
    weight = torch.tensor([0.2, 0.8, 0.2], dtype=torch.float)
...
def compute_pitch_loss(output, target_class, target_regr, mask):
    batch_size = target_regr.shape[0]

    output = output.contiguous()

    loss_class = compute_class_loss(output[:, 0:3, :, :], target_class[:, 0, :, :], mask)

    loss_res = torch.zeros_like(loss_class)

    index = mask.nonzero()

    if len(index) > 0:
        for i in range(len(index)):
            if target_class[index[i][0], 0, index[i][1], index[i][2]].item() == 0:
                loss_sin = compute_res_loss(output[index[i][0], 3, index[i][1], index[i][2]],
                                            torch.sin(target_regr[index[i][0], 0, index[i][1], index[i][2]]))
                loss_cos = compute_res_loss(output[index[i][0], 4, index[i][1], index[i][2]],
                                            torch.cos(target_regr[index[i][0], 0, index[i][1], index[i][2]]))
            if target_class[index[i][0], 0, index[i][1], index[i][2]].item() == 1:
                loss_sin = compute_res_loss(output[index[i][0], 5, index[i][1], index[i][2]],
                                            torch.sin(target_regr[index[i][0], 1, index[i][1], index[i][2]]))
                loss_cos = compute_res_loss(output[index[i][0], 6, index[i][1], index[i][2]],
                                            torch.cos(target_regr[index[i][0], 1, index[i][1], index[i][2]]))
            if target_class[index[i][0], 0, index[i][1], index[i][2]].item() == 2:
                loss_sin = compute_res_loss(output[index[i][0], 7, index[i][1], index[i][2]],
                                            torch.sin(target_regr[index[i][0], 2, index[i][1], index[i][2]]))
                loss_cos = compute_res_loss(output[index[i][0], 8, index[i][1], index[i][2]],
                                            torch.cos(target_regr[index[i][0], 2, index[i][1], index[i][2]]))

            loss_res += loss_cos + loss_sin

    return loss_class + loss_res

    # regr_names = mask  ['x', 'y', 'z', 'yaw', 'roll']


def criter_objective(prediction, mask, regr, pitch_class, pitch_regr, size_average=True):
    pred_mask = torch.sigmoid(prediction[:, 0])
    ...
    pitch_loss = compute_pitch_loss(prediction[:, 6:], pitch_class, pitch_regr, mask)
    # Sum
    loss = w_mask * mask_loss + w_regr * regr_loss + w_pitch * pitch_loss
    if not size_average:
        loss *= prediction.shape[0]
    return loss
...
def evaluate(model, epoch, history=None):
    model.eval()
    val_loss, mask_loss, regr_loss, pitch_loss = torch.tensor(0., requires_grad=False), torch.tensor(0.,
                                                                                                     requires_grad=False), torch.tensor(
        0., requires_grad=False), torch.tensor(0., requires_grad=False)

    with torch.no_grad():
        for img_batch, mask_batch, regr_batch, pitch_class_batch, pitch_res_pitch, _ in dev_loader:
            img_batch = img_batch.to(device)
            mask_batch = mask_batch.to(device)
            regr_batch = regr_batch.to(device)
            pitch_class_batch = pitch_class_batch.to(device)
            pitch_res_pitch = pitch_res_pitch.to(device)

            output = model(img_batch)

            val_loss_batch, mask_loss_batch, regr_loss_batch, pitch_loss_batch = criter_metric(output, mask_batch,
                                                                                               regr_batch,
                                                                                               pitch_class_batch,
                                                                                               pitch_res_pitch,
                                                                                               size_average=False)
    ...
    pitch_loss /= len(dev_loader.dataset)

    return val_loss.cpu().numpy(), mask_loss.cpu().numpy(), regr_loss.cpu().numpy(), pitch_loss.cpu().numpy()
...
def train_eval_model(model, epoch, best_val_loss, history=None):
    ...
    #     x y z yaw roll
    for batch_idx, (img_batch, mask_batch, regr_batch, pitch_class_batch, pitch_regr_batch, _) in enumerate(
            tqdm(train_loader)):
        img_batch = img_batch.to(device)
        mask_batch = mask_batch.to(device)
        regr_batch = regr_batch.to(device)
        pitch_class_batch = pitch_class_batch.to(device)
        pitch_regr_batch = pitch_regr_batch.to(device)

        output = model(img_batch)
        loss = criter_objective(output, mask_batch, regr_batch, pitch_class_batch, pitch_regr_batch)
        ...
        cond2 = abs(batch_idx - len(train_loader)) * 10 > len(
            train_loader)  # if not too close to the end or start of epoch
        is_end_epoch = ((batch_idx + 1) % len(train_loader)) == 0

        if is_end_epoch | (cond1 & cond2):
            t_eval0 = time.time()
            val_loss, mask_val_loss, regr_val_loss, pitch_val_loss = evaluate(model, dev_loader)

            # scheduler.step(val_loss)  for LRONPlateu scheduler
            print("\n=========================")
            print(f"epoch={epoch}, batch={batch_idx} => lr={lr_value:.7f}, trn loss={loss:4f}")
            print(
                f"VAL LOSS: {val_loss:.2f}, \tBest Val Loss: {best_val_loss:.2f} \teval time={(time.time() - t_eval0) / 60:.2f} min.")
            print("mask_val_loss:", mask_val_loss, "\tregr_val_loss", regr_val_loss, "\tpitch_val_loss", pitch_val_loss)
            if is_end_epoch: print(f"\n===END of EPOCH {epoch}================")

            history.loc[epoch * len(train_loader) + batch_idx, ['train_loss', 'val_loss', 'lr', 'mask_val_loss',
                                                                'regr_val_loss', 'pitch_val_loss']] = \
                [train_loss, val_loss, lr_value, mask_val_loss, regr_val_loss, pitch_val_loss]
...
scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[7, 10, 14, 17, 20],
                                     gamma=0.5)  # milestones=[5,7,8], gamma=0.1)
...
if train_control:
    ...
    history = pd.DataFrame(np.full((len(train_loader) * n_epochs, 6), fill_value=-1, dtype=np.float32),
                           columns=['train_loss', 'val_loss', 'lr', 'mask_val_loss', 'regr_val_loss', 'pitch_val_loss'])
    ...
    for epoch in range(epoch_start, n_epochs + epoch_start):
        end_time = time.time()
        print('Time spend:--------%.2f' % ((end_time - now_time) / 60.0))
        ...
        if (val_loss < prev_epoch_val_loss) | (is_last_epoch):
            ...
            print(
                f"Val_loss improved or end training => Saving at epoch {epoch} end w val_loss={val_loss:.2f}, best_val_loss = {best_val_loss:.2f}")
            save_checkpoint(params_save_model,
                            model_name=f"model_fld_{fld}_islastepoch_{is_last_epoch:d}_epoch_{epoch + 1}.pth")
...
def predict_test(model, threshold=0):  # -1.2  # -1.0) #-0.5)
    ...
    model.eval()
    for img_batch, _, _, _, _, test_mask_batch in tqdm(test_loader):
        with torch.no_grad():
            ...
            for out, test_mask in zip(output, test_mask_batch):
                ...
                if isinstance(test_mask, np.ndarray):
                    ...
                    test_mask = cv2.resize(test_mask, (128, 60))[..., 0]
...
if test_control:
    ...
    numcars = pd.DataFrame(
        {'ImageId': sub['ImageId'].values,
         'numcars': [int((str(x).count(' ') + 1) / 7) for x in sub['PredictionString']]})
    numcars_before = pd.DataFrame({'ImageId': sub_before['ImageId'].values,
                                   'numcars': [int((str(x).count(' ') + 1) / 7) for x in
                                               sub_before['PredictionString']]})
