...
def get_alpha(rot):
    ...
    alpha1 = np.arctan(rot[2, :, :] / rot[3, :, :]) -  np.pi
...
class MyUNet(nn.Module):
    ...
    def __init__(self, t_num, r_num, roll_num, drop_rate):
    ...
    def forward(self, x):
        ...
        x = torch.cat((x_t, x_r, x_roll), dim=1)
        return x
...
def build_model(drop_rate):
    ...
    print("device:", device)
    model = MyUNet(T_class, R_class, roll_class, drop_rate).to(device)
