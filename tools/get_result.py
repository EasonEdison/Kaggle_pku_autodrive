...
################################
img_h, img_w = 700, 1600
batch_size = 1
num_epochs = 30
num_epochs_resume = 0
start_epoch = 0
device = torch.device("cuda:1")
out_name = 'test_branch_roll'
model_path = '/home/ql-b423/Desktop/TXH/RB/OP_test_branch_roll/model_fld_3_islastepoch_0_epoch_8.pth'
Train = False
...
#    regr_names = ['x', 'y', 'z', 'yaw', 'pitch_cos', 'pitch_sin']
def extract_coords(prediction, threshold=0):
    ...
    col_names = ['x', 'y', 'z', 'yaw', 'pitch_cos', 'pitch_sin', 'roll']
...
def _regr_preprocess(regr_dict):
    ...
    if depth_loss_control:
        regr_dict['z'] = 1 / sigmoid_(regr_dict['z']) - 1
    # depth not need change
...
def get_mask_and_regr(img_orig_height, img_orig_width, labels):
    ...
    regr = np.zeros([img_h // model_scale, img_w // model_scale, 6], dtype='float32')
    rot_bin = np.zeros([img_h // model_scale, img_w // model_scale, 2], dtype='long')
    rot_res = np.zeros([img_h // model_scale, img_w // model_scale, 2], dtype='float32')
...
class CarDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.training:
            mask, regr, rot_bin, rot_res = get_mask_and_regr(img_orig_h, img_orig_w, labels)
            ...
            mask, regr, rot_bin, rot_res = 0, 0, 0, 0
        ...
        rot_bin = torch.as_tensor(rot_bin, dtype=torch.long)
        rot_res = torch.as_tensor(rot_res, dtype=torch.float32)
        dropmask = torch.as_tensor(dropmask, dtype=torch.float32)

        return [img, mask, regr, rot_bin, rot_res, dropmask]
...
class MyUNet(nn.Module):
    ...
    def __init__(self, tr_num,  roll_num, drop_rate):
        ...
        if effnet_ver == 'b0':
            self.up1_tr = up(1282 + 1024, 512)
        elif effnet_ver == 'b2':
            self.up1_tr = up(1410 + 1024, 512)
        elif effnet_ver == 'b4':
            self.up1_tr = up(1794 + 1024, 512)
        elif effnet_ver == 'b5':
            self.up1_tr = up(2050 + 1024, 512)
        ...
        if effnet_ver == 'b0':
            self.up1_roll = up(1282 + 1024, 512)
        elif effnet_ver == 'b2':
            self.up1_roll = up(1410 + 1024, 512)
        elif effnet_ver == 'b4':
            self.up1_roll = up(1794 + 1024, 512)
        elif effnet_ver == 'b5':
            self.up1_roll = up(2050 + 1024, 512)

        self.up2_tr = up(512 + 512, 256)
        # self.up2_r = up(512 + 512, 256)
        self.up2_roll = up(512 + 512, 256)

        self.outTR = nn.Conv2d(256, self.tr_num, 1)
        # self.outR = nn.Conv2d(256, self.r_num, 1)
        self.outRoll = nn.Conv2d(256, self.roll_num, 1)
    ...
    def forward(self, x):
        ...
        x_tr = self.up1_tr(feats, x4)
        x_tr = self.up2_tr(x_tr, x3)
        x_tr = self.outTR(x_tr)

        # x_r = self.up1_r(feats, x4)
        # x_r = self.up2_r(x_r, x3)
        # x_r = self.outR(x_r)

        x_roll = self.up1_roll(feats, x4)
        x_roll = self.up2_roll(x_roll, x3)
        x_roll = self.outRoll(x_roll)
        x = torch.cat((x_tr, x_roll), dim=1)
        return x
...
def build_model(drop_rate):
    ...
    print("device:", device)
    model = MyUNet(TR_class, roll_class, drop_rate).to(device)
...
def criter_metric(prediction, mask, regr, rot_bin, rot_res, size_average=True):
    # Binary mask loss
    ...
    # Regression L1 loss
    if depth_loss_control:
        prediction[:, 3] = 1 / torch.sigmoid(prediction[:, 3]) - 1
    ...
    # Sum
    loss = mask_loss + regr_loss + rot_bin_loss
    if not size_average:
        loss *= prediction.shape[0]
    return loss, mask_loss, regr_loss, rot_bin_loss
...
def compute_rot_loss(output, target_bin, target_res, mask):
    ...
    # xxx = 12
    # cccc = output[mask_index[xxx][0],0:2,mask_index[xxx][1]]
    # ssss = output[mask_index[xxx][0],4:6,mask_index[xxx][1]]
    # aaaa = target_bin[mask_index[xxx][0],:,mask_index[xxx][1]]
    # bbbb = target_res[mask_index[xxx][0],:,mask_index[xxx][1]]
    mask = mask.reshape(batch_size, -1)
    ########################test
    ...
    if target_bin[:, 0, :].nonzero().shape[0] > 0:
        ...
        for i in range(len(idx1)):
            # 将对应的真实值也挑选了出来
            # 这里应该就是常规的算正样本的loss
            loss_sin1 = compute_res_loss(output[idx1[i][0], 2, idx1[i][1]],
                                         torch.sin(target_res[idx1[i][0], 0, idx1[i][1]]))
            loss_cos1 = compute_res_loss(output[idx1[i][0], 3, idx1[i][1]],
                                         torch.cos(target_res[idx1[i][0], 0, idx1[i][1]]))
            loss_res += loss_sin1 + loss_cos1
    if target_bin[:, 1, :].nonzero().shape[0] > 0:
        idx2 = target_bin[:, 1, :].nonzero()
        for i in range(len(idx2)):
            loss_sin2 = compute_res_loss(output[idx2[i][0], 6, idx2[i][1]],
                                         torch.sin(target_res[idx2[i][0], 1, idx2[i][1]]))
            loss_cos2 = compute_res_loss(output[idx2[i][0], 7, idx2[i][1]],
                                         torch.cos(target_res[idx2[i][0], 1, idx2[i][1]]))
...
# regr_names = ['x', 'y', 'z', 'yaw', 'pitch_cos','pitch_sin']
def criter_objective(prediction, mask, regr, rot_bin, rot_res, size_average=True):
    pred_mask = torch.sigmoid(prediction[:, 0])
    ...
    # Sum
    loss = w_mask * mask_loss + w_regr * regr_loss + w_roll * rot_bin_loss
    if not size_average:
        loss *= prediction.shape[0]
    return loss
...
def evaluate(model, epoch, history=None):
    model.eval()
    val_loss, mask_loss, regr_loss, roll_loss = torch.tensor(0., requires_grad=False), torch.tensor(0.,
                                                                                                    requires_grad=False), torch.tensor(
        0., requires_grad=False), torch.tensor(0., requires_grad=False)

    with torch.no_grad():
        for img_batch, mask_batch, regr_batch, rot_bin_batch, rot_res_batch, _ in dev_loader:
            ...
            val_loss_batch, mask_loss_batch, regr_loss_batch, roll_loss_batch = criter_metric(output, mask_batch,
                                                                                              regr_batch, rot_bin_batch,
                                                                                              rot_res_batch,
                                                                                              size_average=False)
            val_loss += val_loss_batch
            mask_loss += mask_loss_batch
            regr_loss += regr_loss_batch
            roll_loss += roll_loss_batch

    val_loss /= len(dev_loader.dataset)
    mask_loss /= len(dev_loader.dataset)
    regr_loss /= len(dev_loader.dataset)
    roll_loss /= len(dev_loader.dataset)

    return val_loss.cpu().numpy(), mask_loss.cpu().numpy(), regr_loss.cpu().numpy(), roll_loss.cpu().numpy()
...
def train_eval_model(model, epoch, best_val_loss, history=None):
    ...
    for batch_idx, (img_batch, mask_batch, regr_batch, rot_bin_batch, rot_res_batch, _) in enumerate(
            tqdm(train_loader)):
        img_batch = img_batch.to(device)
        mask_batch = mask_batch.to(device)
        regr_batch = regr_batch.to(device)
        ...
        output = model(img_batch)
        loss = criter_objective(output, mask_batch, regr_batch, rot_bin_batch, rot_res_batch)
        ...
        if is_end_epoch | (cond1 & cond2):
            t_eval0 = time.time()
            val_loss, mask_val_loss, regr_val_loss, roll_val_loss = evaluate(model, dev_loader)
            ...
            print("mask_val_loss:", mask_val_loss, "\tregr_val_loss", regr_val_loss, "\troll_val_loss", roll_val_loss)
            if is_end_epoch: print(f"\n===END of EPOCH {epoch}================")

            history.loc[epoch * len(train_loader) + batch_idx, ['train_loss', 'val_loss', 'lr', 'mask_val_loss',
                                                                'regr_val_loss', 'roll_val_loss']] = \
                [train_loss, val_loss, lr_value, mask_val_loss, regr_val_loss, roll_val_loss]
...
val_idx_t = np.array((0,1))
train_dataset = CarDataset(train[['ImageId', 'PredictionString']], train_images_dir,
                           transform=albu_transform_train)
dev_dataset = CarDataset(train[['ImageId', 'PredictionString']].iloc[val_idx_t], train_images_dir,
...
if train_control:
    ...
    history = pd.DataFrame(np.full((len(train_loader) * n_epochs, 6), fill_value=-1, dtype=np.float32),
                           columns=['train_loss', 'val_loss', 'lr', 'mask_val_loss', 'regr_val_loss', 'roll_val_loss'])
        ...
        with torch.no_grad():
            ...
            for out, test_mask in zip(output, test_mask_batch):
                ...
                if isinstance(test_mask, np.ndarray):
                    ...
                    test_mask = cv2.resize(test_mask, (200, 87))[..., 0]
